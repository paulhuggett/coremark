...
static int stdout_flush(FILE *f) {
  (void)f;
  if (pos > 0) {
...
static FILE __stdio = FDEV_SETUP_STREAM(stdout_writec, NULL, stdout_flush, FDEV_SETUP_WRITE);
FILE *const stdout = &__stdio;
__strong_reference(stdout, stdin);

static FILE __stdio_stderr = FDEV_SETUP_STREAM(stderr_writec, NULL, NULL, FDEV_SETUP_WRITE);
...
int sys_semihost_get_cmdline(char *buf, size_t size) {
  uint64_t args[64];
  uint64_t const res = syscall(sys_getmainvars, (uintptr_t)args, sizeof(args), 0, 0, 0, 0, 0);
  if (res != 0) {
    // args buffer overflowed. Return an error to picolibc.
    return -1;
  }

  // Turn the array of separate args that was returned by spike into a single
  // space-separated array for picolibc.
  char *orig_buf = buf;
  ...
  for (unsigned ctr = 0; ctr < argc; ++ctr) {
    ...
    size -= len;
    if (size == 0) {
      // No space left. Return an error to picolibc.
      return -1;
    }
  }
  *buf = '\0';
...
static void file_decref(file_t* f) {
  if (f->refcnt-- == 2) {
    f->refcnt = 0;
    syscall(sys_close, f->kfd, 0, 0, 0, 0, 0, 0);
  }
}
